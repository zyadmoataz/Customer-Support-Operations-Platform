...
const statusMap: Record<TicketStatus, { label: string; bg: string; text: string; dot: string }> = {
  open: { label: 'Open • In Queue', bg: 'bg-sky-500/10 border-sky-500/25', text: 'text-sky-400', dot: 'bg-sky-400' },
  in_progress: { label: 'In Progress • Under Investigation', bg: 'bg-amber-500/10 border-amber-500/25', text: 'text-amber-400', dot: 'bg-amber-400' },
  resolved: { label: 'Resolved & Verified', bg: 'bg-emerald-500/10 border-emerald-500/25', text: 'text-emerald-400', dot: 'bg-emerald-400' },
}
        ...
        >
          ...
          <div className="flex items-start justify-between gap-4 pb-4 border-b border-slate-800">
            <div>
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold border ${status.bg} ${status.text}`}>
            ...
            <button onClick={onClose} className="p-1.5 rounded-xl bg-slate-900 text-slate-400 hover:text-white border border-slate-800 transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 my-5 text-xs">
            <div className="p-3 rounded-2xl bg-slate-900/80 border border-white/5">
              ...
              <span className="font-semibold text-white">{ticket.category || 'General Inquiries'}</span>
            </div>
            <div className="p-3 rounded-2xl bg-slate-900/80 border border-white/5">
              <span className="text-slate-400 block mb-1">Priority Level</span>
              <span className="font-bold uppercase tracking-wider text-amber-400">{ticket.priority}</span>
            </div>
