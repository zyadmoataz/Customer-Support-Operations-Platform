      ...
      {isLoading ? (
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
          {[1, 2, 3, 4].map((n) => (
            <div
              key={n}
              className='h-44 rounded-3xl glass-panel p-6 border border-white/5 flex flex-col justify-between'
            >
        ...
        <div className='p-12 rounded-3xl glass-panel text-center border border-dashed border-slate-800'>
          ...
          <p className='text-xs text-slate-400 mt-1 max-w-sm mx-auto leading-relaxed'>
            {searchQuery
              ...
              : "You have no tickets currently matching this filter status."}
          </p>
        </div>
      ) : (
